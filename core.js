...
(function(global) {
    var validTypes = ["text", "search", "url", "tel", "email", "password", "number", "textarea"],
        ...
        inputs, textareas, ATTR_CURRENT_VAL = "data-placeholder-value",
    ...
    function hidePlaceholder(elem,
        keydownValue) {
        var type, maxLength, valueChanged = !! keydownValue && elem.value !== keydownValue,
    ...
    function showPlaceholder(elem) {
        ...
        if (elem.value === "" && val) {
            ...
            if (type) elem.type = "text";
                ...
                if (Utils.changeType(elem, "text")) elem.setAttribute(ATTR_INPUT_TYPE, "password");
            return true
        }
        return false
    }
    ...
    function makeFocusHandler(elem) {
        return function() {
            if (hideOnInput && (elem.value === elem.getAttribute(ATTR_CURRENT_VAL) && elem.getAttribute(ATTR_ACTIVE) ===
                "true")) Utils.moveCaret(elem, 0);
    ...
    function makeClickHandler(elem) {
        return function() {
            if (elem === safeActiveElement() && (elem.value === elem.getAttribute(ATTR_CURRENT_VAL) && elem.getAttribute(ATTR_ACTIVE) === "true")) Utils.moveCaret(elem, 0)
        }
    }
    ...
    function newElement(elem) {
        ...
        if (hideOnInput || elem !== safeActiveElement()) showPlaceholder(elem)
    }
    ...
    if (!Placeholders.nativeSupport) {
        ...
        if (styleElem.styleSheet) styleElem.styleSheet.cssText =
            styleRules.nodeValue;
        else styleElem.appendChild(styleRules);
        ...
        for (i = 0, len = inputs.length + textareas.length; i < len; i++) {
            ...
            if (placeholder) {
                ...
                if (placeholder && Utils.inArray(validTypes, elem.type)) newElement(elem)
            }
        }
        timer = setInterval(function() {
            for (i = 0, len = inputs.length + textareas.length; i < len; i++) {
                elem = i < inputs.length ? inputs[i] : textareas[i -
                    inputs.length];
                placeholder = elem.attributes.placeholder;
                if (placeholder) {
                    placeholder = placeholder.nodeValue;
                    if (placeholder && Utils.inArray(validTypes, elem.type)) {
                        if (!elem.getAttribute(ATTR_EVENTS_BOUND)) newElement(elem);
                        if (placeholder !== elem.getAttribute(ATTR_CURRENT_VAL) || elem.type === "password" && !elem.getAttribute(ATTR_INPUT_TYPE)) {
                            if (elem.type === "password" && (!elem.getAttribute(ATTR_INPUT_TYPE) && Utils.changeType(elem, "text"))) elem.setAttribute(ATTR_INPUT_TYPE, "password");
                            if (elem.value === elem.getAttribute(ATTR_CURRENT_VAL)) elem.value =
                                placeholder;
                            elem.setAttribute(ATTR_CURRENT_VAL, placeholder)
                        }
                    }
            ...
            if (!liveUpdates) clearInterval(timer)
        }, 100)
    }
...
(function($) {
    ...
    if (!Placeholders.nativeSupport) {
        $.fn.val = function(val) {
            ...
            if (val === undefined && (this.eq(0).data("placeholder-active") && originalValue === placeholder)) return "";
            return originalValue
        };
        $.fn.prop = function(name, val) {
            if (val === undefined && (this.eq(0).data("placeholder-active") && name === "value")) return "";
    ...
    if (chromeVersion)
        if (deviceIsAndroid) {
            ...
            if (metaViewport) {
                ...
                if (chromeVersion > 31 && window.innerWidth <= window.screen.width) return true
            }
        } else return true;
...
/* ========================================================================
 * Unveil.js v??
 * PICTURE LOADER (INCLUDING RETINA)
...
(function() {
    ...
    (function(root, factory) {
        if (typeof define === "function" && define.amd) return define("waypoints", ["jquery"], function($) {
            return factory($, root)
        });
        else return factory(root.jQuery, root)
    })(this, function($, window) {
            var $w, Context, Waypoint, allWaypoints, contextCounter, contextKey, contexts, isTouch, jQMethods, methods, resizeEvent, scrollEvent, waypointCounter, waypointKey,
                wp, wps;
            $w = $(window);
            ...
            Context = function() {
                ...
                Context.prototype.doScroll = function() {
                    ...
                    if (isTouch && (!axes.vertical.oldScroll || !axes.vertical.newScroll)) $[wps]("refresh");
                    $.each(axes, function(aKey, axis) {
                        ...
                        $.each(_this.waypoints[aKey], function(wKey, waypoint) {
                            var _ref, _ref1;
                            if (axis.oldScroll < (_ref = waypoint.offset) && _ref <= axis.newScroll) return triggered.push(waypoint);
                            else if (axis.newScroll < (_ref1 = waypoint.offset) && _ref1 <= axis.oldScroll) return triggered.push(waypoint)
                        });
                        triggered.sort(function(a, b) {
                            return a.offset - b.offset
                        });
                        if (!isForward) triggered.reverse();
                ...
                Context.prototype.refresh = function() {
                    ...
                    return $.each(axes, function(aKey, axis) {
                        return $.each(_this.waypoints[aKey], function(i, waypoint) {
                            ...
                            elementOffset =
                                $.isWindow(waypoint.element) ? 0 : waypoint.$element.offset()[axis.offsetProp];
                            if ($.isFunction(adjustment)) adjustment = adjustment.apply(waypoint.element);
                            else if (typeof adjustment === "string") {
                                adjustment = parseFloat(adjustment);
                                if (waypoint.options.offset.indexOf("%") > -1) adjustment = Math.ceil(axis.contextDimension * adjustment / 100)
                            }
                            waypoint.offset = elementOffset - axis.contextOffset + axis.contextScroll - adjustment;
                            if (waypoint.options.onlyOnScroll && oldOffset != null || !waypoint.enabled) return;
                            if (oldOffset !== null && (oldOffset <
                                (_ref = axis.oldScroll) && _ref <= waypoint.offset)) return waypoint.trigger([axis.backward]);
                            else if (oldOffset !== null && (oldOffset > (_ref1 = axis.oldScroll) && _ref1 >= waypoint.offset)) return waypoint.trigger([axis.forward]);
            ...
            Waypoint = function() {
                function Waypoint($element, context, options) {
                    ...
                    if (options.offset === "bottom-in-view") options.offset = function() {
                        var contextHeight;
                        contextHeight = $[wps]("viewportHeight");
                        if (!$.isWindow(context.element)) contextHeight = context.$element.height();
                        return contextHeight - $(this).outerHeight()
                    };
                    this.$element = $element;
                    this.element = $element[0];
                    this.axis = options.horizontal ? "horizontal" :
                        "vertical";
            ...
            methods = {
                init: function(f, options) {
                    ...
                    this.each(function() {
                        ...
                        if (!$.isWindow(contextElement)) contextElement = $this.closest(contextElement);
                        contextElement = $(contextElement);
                        context = contexts[contextElement[0][contextKey]];
                        if (!context) context = new Context(contextElement);
                ...
                next: function(axis, selector) {
                    return methods._traverse.call(this, axis, selector, function(stack, index,
                        waypoints) {
            ...
            $.fn[wp] = function() {
                ...
                if (methods[method]) return methods[method].apply(this, args);
                else if ($.isFunction(method)) return methods.init.apply(this, arguments);
                else if ($.isPlainObject(method)) return methods.init.apply(this, [null, method]);
                else if (!method) return $.error("jQuery Waypoints needs a callback function or handler option.");
                else return $.error("The " + method + " method does not exist in jQuery Waypoints.")
            };
            $.fn[wp].defaults = {
                context: window,
                continuous: true,
            ...
            jQMethods = {
                ...
                above: function(contextSelector) {
                    if (contextSelector == null) contextSelector =
                        window;
                ...
                below: function(contextSelector) {
                    if (contextSelector == null) contextSelector = window;
                    return jQMethods._filter(contextSelector, "vertical", function(context, waypoint) {
                        return waypoint.offset > context.oldScroll.y
                    })
                },
                ...
                _invoke: function(method) {
                    var waypoints;
                    waypoints =
                        $.extend({}, allWaypoints.vertical, allWaypoints.horizontal);
            ...
            $[wps] = function() {
                var args, method;
                method = arguments[0], args = 2 <= arguments.length ? __slice.call(arguments, 1) : [];
                if (jQMethods[method]) return jQMethods[method].apply(null, args);
                else return jQMethods.aggregate.call(null, method)
            };
            $[wps].settings = {
                resizeThrottle: 100,
                scrollThrottle: 30
 ...
 * ======================================================================== */
(function(factory) {
    if (typeof module === "function") module.exports = factory(this.jQuery || require("jquery"));
    else if (typeof define === "function" && define.amd) define(["jquery"], function($) {
        return factory($)
    });
    ...
    NProgress.getPositioningCSS = function() {
        ...
        var vendorPrefix = "WebkitTransform" in bodyStyle ?
            "Webkit" : "MozTransform" in bodyStyle ? "Moz" : "msTransform" in bodyStyle ? "ms" : "OTransform" in bodyStyle ? "O" : "";
    ...
    function barPositionCSS(n, speed, ease) {
        ...
        if (Settings.positionUsing === "translate3d") barCSS = {
            transform: "translate3d(" + toBarPerc(n) +
                "%,0,0)"
        };
...
(function($) {
    ...
    for (var key in support)
        if (support.hasOwnProperty(key) && typeof $.support[key] === "undefined") $.support[key] =
            support[key];
    div = null;
    $.cssEase = {
        "_default": "ease",
    ...
    $.cssHooks["transit:transform"] = {
        ...
        set: function(elem, v) {
            ...
            if (support.transform === "WebkitTransform" && !isChrome) elem.style[support.transform] = value.toString(true);
            else elem.style[support.transform] =
                value.toString();
    ...
    Transform.prototype = {
        setFromString: function(prop, val) {
            var args = typeof val ===
                "string" ? val.split(",") : val.constructor === Array ? val : [val];
            args.unshift(prop);
        ...
        setter: {
            ...
            translate: function(x, y) {
                ...
                if (y !==
                    null && y !== undefined) this._translateY = unit(y, "px");
        ...
        toString: function(use3d) {
            ...
            for (var i in this)
                if (this.hasOwnProperty(i)) {
                    ...
                    if (use3d && i === "translate") re.push(i + "3d(" + this[i] + ",0)");
    ...
    function callOrQueue(self, queue,
        fn) {
...
/* ========================================================================
 * Response.js v0.8.0
 * BREAKPOINTS IN JAVASCRIPT FOR REPSONSIVE SITES
...
(function(root, name, make) {
    ...
    function map(stack, fn, scope) {
        for (var r = [], l = stack.length, i = 0; i < l;) r[i] = fn.call(scope, stack[i], i++,
            stack);
        return r
    }
    ...
    function affix(stack, prefix, suffix) {
        ...
        for (var r = [], l = stack.length, i = 0; i < l; i++) null == stack[i] || r.push(prefix + stack[i] + suffix);
        return r
    }
    ...
    function merge(r, s) {
        ...
        if (typeof s == "object" && isNumber(s.length)) push.apply(r, sift(s, "undefined", true));
            ...
            for (var k in s) owns.call(s, k) && (void 0 !== s[k] && (r[k] = s[k]));
        return r
    }

    function route(item, fn, scope) {
        if (null == item) return item;
        if (typeof item == "object" && (!item.nodeType && isNumber(item.length))) each(item,
            fn, scope);
    ...
    function dpr(decimal) {
        ...
        if (null == decimal) return dPR || (dpr(2) ? 2 : dpr(1.5) ? 1.5 : dpr(1) ? 1 : 0);
    ...
    function render(s) {
        var n;
        return typeof s != "string" || !s ? s : "false" === s ? false : "true" === s ? true : "null" === s ? null : "undefined" === s || ((n = +s) || (0 === n || "NaN" === s)) ? n : s
    }

    function getNative(e) {
        return !e ?
            false : e.nodeType === 1 ? e : e[0] && e[0].nodeType === 1 ? e[0] : false
    }

    function datasetChainable(key, value) {
        var n, numOfArgs = arguments.length,
            elem = getNative(this),
        ...
        if (numOfArgs) {
            ...
            if (typeof key === "string") {
                ...
                if (this === elem || 2 > (n = this.length || 1)) elem.setAttribute(key, value);
                    ...
                    while (n--) n in this && datasetChainable.apply(this[n], arguments)
            } else if (key instanceof Object)
                for (n in key) key.hasOwnProperty(n) && datasetChainable.call(this, n, key[n]);
            return this
        }
    ...
    function dataset(elem) {
        return datasetChainable.apply(elem,
    ...
    function sel(keys) {
        for (var k, r = [], i = 0, l = keys.length; i < l;)(k = keys[i++]) && r.push("[" + datatize(k.replace(regexTrimPunc, "").replace(".", "\\.")) + "]");
        return r.join()
    }
    ...
    function inViewport(elem, verge) {
        var r = rectangle(getNative(elem), verge);
        return !!r && (r.bottom >= 0 && (r.top <= viewportH() && (r.right >= 0 && r.left <=
            viewportW())))
    }
    ...
    Elemset = function() {
        ...
        return {
            ...
            configure: function(options) {
                merge(this, options);
                var i, points, prefix, aliases, aliasKeys, isNumeric = true,
                    prop = this.prop;
                this.uid = suid++;
                ...
                } else points = defaultPoints[prop] || (defaultPoints[prop.split("-").pop()] || doError("create @prop"));
                ...
                if (aliases) {
                    ...
                    this.aka = aliasKeys;
                    this.keys = concat.apply(this.keys,
                        aliasKeys)
                }
        ...
        route(args, function(options) {
            ...
            ready(function() {
                ...
                each(elemset.target().$e, function(el, i) {
                    elemset[i] = procreate(elemset).prepareData(el);
                    if (!lazy || inViewport(elemset[i].$e, verge)) elemset[i].updateDOM()
                });
                if (elemset.dynamic && (elemset.custom || lowestNonZeroBP < screenMax)) resize(resizeHandler, resizeName);
                if (!lazy) return;
                $win.on(scrollName, scrollHandler);
                elemset.$e.one(allLoaded,
                    function() {
 ...
 * ======================================================================== */
(function(root, factory) {
    if (typeof exports === "object" && exports) factory(exports);
    else {
        var mustache = {};
        factory(mustache);
        if (typeof define === "function" && define.amd) define(mustache);
    ...
    var isArray = Array.isArray || function(object) {
            return Object_toString.call(object) ===
                "[object Array]"
        };
    ...
    function escapeTags(tags) {
        ...
        return [new RegExp(escapeRegExp(tags[0]) +
            "\\s*"), new RegExp("\\s*" + escapeRegExp(tags[1]))]
    }
    var whiteRe = /\s*/;
    var spaceRe = /\s+/;
    var equalsRe = /\s*=/;
    ...
    function parseTemplate(template, tags) {
        ...
        while (!scanner.eos()) {
            ...
            if (type === "=") {
                ...
                scanner.scanUntil(tagRes[1]);
                type = "&"
            } else value = scanner.scanUntil(tagRes[1]); if (!scanner.scan(tagRes[1])) throw new Error("Unclosed tag at " + scanner.pos);
            token = [type, value, start, scanner.pos];
            tokens.push(token);
            if (type === "#" || type === "^") sections.push(token);
            else if (type === "/") {
                openSection = sections.pop();
                if (!openSection) throw new Error('Unopened section "' +
                    value + '" at ' + start);
                if (openSection[1] !== value) throw new Error('Unclosed section "' + openSection[1] + '" at ' + start);
            } else if (type === "name" || (type === "{" || type === "&")) nonSpace = true;
            else if (type === "=") tagRes = escapeTags(tags = value.split(spaceRe))
        }
        openSection = sections.pop();
    ...
    Context.prototype.lookup = function(name) {
        var value;
        if (name in this.cache) value = this.cache[name];
        else {
            var context = this;
            while (context) {
                if (name.indexOf(".") > 0) {
                    ...
                    var names = name.split("."),
                        i = 0;
                    while (value != null && i < names.length) value = value[names[i++]]
                } else value = context.view[name]; if (value != null) break;
        ...
        for (var i =
            ...
            switch (token[0]) {
                case "#":
                    ...
                    if (!value) continue;
                    if (isArray(value))
                        for (var j = 0, jlen = value.length; j < jlen; ++j) buffer += this.renderTokens(token[4], context.push(value[j]), partials, originalTemplate);
                    else if (typeof value === "object" || typeof value === "string") buffer += this.renderTokens(token[4], context.push(value), partials, originalTemplate);
    ...
    mustache.to_html = function(template, view, partials, send) {
        ...
        if (isFunction(send)) send(result);
        else return result
    };
        ...
        PLUGINS: function() {
            ...
            (function() {
                ...
                function selectrow($this, state) {
                    ...
                    if (state === "checked") {
                        ...
                        // publish event
                        $(element).trigger(settings.eventPrefix + ".selectrow.selected", {
                            "element": $($this).parents(target)
                        });
                    } else {
            ...
            (function() {
                ...
                function checked() {
                    ...
                    // publish event
                    $(element).trigger(settings.eventPrefix + ".checkall.checked", {
                        "element": $(target)
                    });
                }
                ...
                function unchecked() {
                    ...
                    // publish event
                    $(element).trigger(settings.eventPrefix + ".checkall.unchecked", {
                        "element": $(target)
                    });
                }
            ...
            (function() {
                ...
                $(element).on("click", toggler, function(e) {
                    ...
                    if (indicator.length !== 0) {
                        ...
                        // publish event
                        $(element).trigger(settings.eventPrefix + ".panelrefresh.refresh", {
                            "element": $(panel)
                        });
                    } else {
            ...
            (function() {
                ...
                $(element).on("click", toggler, function(e) {
                    ...
                    function open(toggler) {
                        ...
                        // publish event
                        $(element).trigger(settings.eventPrefix + ".panelcollapse.open", {
                            "element": $(panel)
                        });
                    }
                    ...
                    function close(toggler) {
                        ...
                        // publish event
                        $(element).trigger(settings.eventPrefix + ".panelcollapse.close", {
                            "element": $(panel)
                        });
                    }
            ...
            (function() {
                ...
                function toggleShrink(e) {
                    ...
                    if ($(element).hasClass("sidebar-minimized")) {
                        ...
                        // publish event
                        $(element).trigger(settings.eventPrefix + ".sidebar.maximize", {
                            "element": $(element)
                        });
                    } else {
                        isShrinked = true;
                        ...
                        // publish event
                        $(element).trigger(settings.eventPrefix + ".sidebar.minimize", {
                            "element": $(element)
                        });
                    }
                ...
                function toggleCollapse(e) {
                    ...
                    if (e.type === "click") {
                        ...
                        var target = $(this).data("target"),
                            self = this,
                            $parent = $(self).parent(),
                            descendants = $parent.find('*').addBack().toArray();
                        ...
                        if ($(target).hasClass("in")) {
                            ...
                            //CHANGE: GJ
                            //for small sidebar: hide submenu when anywhere clicked but in submenu. 
                            //this works without needing to check bc event-bubbling already checks for us. I.e.: if document click is reached
                            //it wasn't captured by the current method (toggleCollapse)
